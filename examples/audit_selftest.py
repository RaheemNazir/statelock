...
def run(name, events, expect_stale, expect_reread=None):
    ...
    print(f"{name:<14} pairs checked {checked:>3}  stale {stale} (planted {expect_stale})"
...
def main():
    ev, planted = mt.clean(60);  run("clean.jsonl", ev, planted, 0)
    ev, planted = mt.stale(60);  run("stale.jsonl", ev, planted, 0)
    ev, planted = mt.mixed(60);  run("mixed.jsonl", ev, planted, 4)
    for seed in range(1, 6):     # the count must hold across sessions, not one lucky seed
        ev, planted = mt.stale(80, planted=9, seed=seed)
        run(f"stale_s{seed}.jsonl", ev, planted, 0)

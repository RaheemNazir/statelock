...
(iv) the same pipeline on a prose task whose control is not
at chance -- a transformer without the machine reaches 0.92 in distribution and
...
(v) a scaling ladder to 88.35M parameters -- GPT-2-small's architecture,
...
(0.503) at every size; and (vi) a training recipe under which the network DISCOVERS a verified-exact
...
without it; and (vii) the bridge: the 120-state machine the 88M model consults is itself induced
...
- Both tasks are synthetic and the prose in Section 7b is templated. No result
